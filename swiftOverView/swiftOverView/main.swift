...
extension Result: Decodable {
    init(from decoder: Decoder) throws {
        ...
        congress = try resultContainer.decode(String.self, forKey: .congress)
        members = try resultContainer.decode([Member].self, forKey: .members)
    }
}

extension Member: Decodable {
    init(from decoder: Decoder) throws {
        let memberContainer = try decoder.container(keyedBy: MemberKeys.self)
        id = try memberContainer.decode(String.self, forKey: .id)
        title = try memberContainer.decode(String.self, forKey: .title)
        first_name = try memberContainer.decode(String.self, forKey: .first_name)
        if let middle_name = try memberContainer.decodeIfPresent(String.self, forKey: .middle_name) {
            self.middle_name = middle_name
        } else {
            self.middle_name = "No Middle Name"
        }
        last_name = try memberContainer.decode(String.self, forKey: .last_name)
        date_of_birth = try memberContainer.decode(String.self, forKey: .date_of_birth)
        party = try memberContainer.decode(String.self, forKey: .party)
        if let twitter_id = try memberContainer.decodeIfPresent(String.self, forKey: .twitter_id) {
            self.twitter_id = twitter_id
        } else {
            self.twitter_id = "No Twitter ID"
        }
    }
}
...
//        print(member.date_of_birth)
//        print(member.party)
//        print(member.twitter_id)
//        print("\n")
//        if member.party == "D" {
//            demCount += 1
//            print("Democratic\(demCount)")
//        }
//    }
//    print("Dem finished\(demCount)")
